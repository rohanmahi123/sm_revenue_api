"""
GET  /models               – list all models for a user
GET  /models/{model_id}    – get model detail
DELETE /models/{model_id}  – delete a model (removes .pkl too)
"""
...
def list_models(
    user_id: str = Query(..., description="Filter by user/tenant ID"),
    dataset_id: int | None = Query(None, description="Filter by dataset"),
    db: Session = Depends(get_db),
):
    q = db.query(TrainedModel).filter_by(user_id=user_id)
